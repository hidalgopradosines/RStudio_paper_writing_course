...
format:
  ...
  pdf:
    ...
    include-before-body:
      text: |
        \linenumbers 
#If i delete from "include before body:" on, I do not get the                        line numbers in the pdf 
bibliography: references.bib #add bibliography in this file
csl: elife.csl #change according to journal
---

```{r include=FALSE}
source("../analysis/scripts/statistics_for_paper.R") ##../ means that we go up one folder in the directory path
...
#| echo: false #the code is not going to be displayed in the final document; if we set it to true, the code is shown
#| #To also hide the warnings we use warning: false
...
$$  this includes a new line \## Sourcing code and working with variable
